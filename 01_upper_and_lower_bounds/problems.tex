...
\begin{problem} \label{prob:sup-inf-claims}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item If $\sup A = s$ and $\sup B = t$, then $\sup (A + B) = \sup A + \sup B$.
      The set $A + B$ is defined as $A + B = \{a + b : a \in A \text{ and } b \in B\}$. \label{prob:additivity-sups}
...
\begin{proof}
  We start by proving two, useful lemmas that will aid us later in the proof.
    ...
    $t = t' + \sqrt{2}$. Then $t \in (a, b)$, and by assumption, $t \in \Q$. By \lemref{lem:rational-irrational-addition-multiplication}, 
    since $\sqrt{2} \notin \Q$, it follows that $t' \in \I$. Therefore, every point in the interval $(a - \sqrt{2}, b - \sqrt{2})$ 
    must be irrational. This contradicts \probref{prob:density-rationals-in-reals}, which states that the rationals are dense 
