...
\begin{proof}
  Without loss of generality, let $x_1, x_2 \in \R$ be arbitrary with $x_1 < x_2$. Suppose, for 
  contradiction, that there are only \textit{finitely} many rational numbers between $x_1$ and $x_2$. Let
  \[
    S = \{r \in \Q : x_1 < r < x_2 \}.
  \]
  Since $S$ is nonempty and finite, there exists a maximum element $M \in S$
  such that $M \geq s$ for all $s \in S$. However, $M < x_2$ and $M, x_2 \in \R$, 
  so by the density of rational numbers in the reals, there exists $M' \in \Q$ 
  such that $M < M' < x_2$. This leads to a contradiction:
  \begin{itemize}
    \item $M$ is the maximum rational number between $x_1$ and $x_2$, but
    \item $M' > M$ and is a rational number between $x_1$ and $x_2$.
  \end{itemize}
  Therefore, our assumption that there are finitely many rational numbers
  between $x_1$ and $x_2$ is false. Hence, there are infinitely many rational
