  ...
  \begin{enumerate}[label=(\alph*)]
    \item Let $g(x) = x^{3}$, and classify each of the following as positive, negative, or zero:
      \begin{enumerate}[label=(\roman*)]
        \item $\int_{0}^{-1} g + \int_{1}^{0} g = 0$, zero.
...
\begin{problem}
  ...
  \begin{proof}
    ...
    \begin{subproof}
      ...
      we still have a finite collection of open intervals that cover $[0, 1]$:
      \[
        S' = \{ ( c_{j}, d_{j} ) : j \in \{ 1, 2, \ldots, m \} \} \supseteq [0, 1],
      \]
      where $m \leq n$. But here we find a contradiction (since $S, S'$ are finite collections of intervals):
      \[
...
\begin{problem}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Review the proof of Theorem 7.4.2 (Abbott 2015), and provide an argument for part (i) of this theorem.
