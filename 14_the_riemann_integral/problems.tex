...
\begin{problem}
  \label{prob:seq-criterion-integr-ex}
  \begin{enumerate}[label=(\alph*)]
    \item 
    ...
    \item Use the sequential criterion for integrability from \ref{prob:seq-criterion-integr} to show directly that 
      $f(x) = x$  is integrable on $[0, 1]$ and compute $\int_{0}^{1} f$.

      Immediately, by the result of \ref{prob:seq-criterion-integr}, we have
      \begin{align*}
