  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Prove that $g$ is continuous at a point $c \neq 0$.
      ...
      \begin{proof}
        ...
        an $m \leq \abs{ \sqrt[3]{x^{2}} + \sqrt[3]{xc} + \sqrt[3]{c^{2}} }$ in
        terms of $c$, which will allow us to make $\abs{\sqrt[3]{x} - \sqrt[3]{c}}$
        arbitrarily small via $\abs{x - c} < \delta$. Bounding $x$ away from
        $0$ ensures that $x$ and $c$ share the same sign (for any $x$ such
        that $\abs{x - c} < \dfrac{\abs{c}}{2}$), thus
        \[
